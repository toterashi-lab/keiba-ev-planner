...
function trainOne(database, agent, now) {
  ...
  if (calibration.status !== "fitted" || !window) return { agentId: agent.id, status: "insufficient", sampleSize: rows.length, settledRaceCount: raceScores.length, minimumSamples: LEARNING_POLICY.minimumSamples, metrics };
  ...
  try {
    database.prepare(`insert into training_runs values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(trainingRunId, agent.id, now, now,
      ...
      artifact.trainingWindow.testStart, artifact.trainingWindow.testEnd, "completed", JSON.stringify({ settledHorseRows: rows.length, settledRaceScores: raceScores.length }), JSON.stringify(metrics), current?.model_version ?? null, artifact.modelVersion);
  ...
  return { agentId: agent.id, status: "candidate_created", sampleSize: rows.length, settledRaceCount: raceScores.length, modelVersion: artifact.modelVersion, metrics };
}
