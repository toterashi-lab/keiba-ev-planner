...
能力モデルの研究利用判定と購入ゲートは分離する。30年の結果データを使った時系列外予測が、一様予測より良いlog loss、ECE 0.025以下、最大校正bin誤差0.075以下、確率合計誤差1e-6以下を全foldで満たした場合だけ `research_pass` とする。この段階では能力モデル由来の研究EVをランキングできるが、過去の発走前オッズ時刻を証明できないため正式な `pass` ではない。市場比較、オッズ鮮度、ROI信頼区間、ドローダウン、最低投票数・日数の全ゲートが合格するまで購入対象外を維持する。
